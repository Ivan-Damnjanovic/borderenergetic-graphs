 ...
 * @version April 2018
 * 
 * Concerning linear algebra libraries:
 * - EJML is faster if you need only eigenvalues, but not eigenvectors
 * - COLT is faster if you need eigenvectors
 * - COLT however cannot compute only eigenvalues without computing eigenvectors
 * Hence COLT is used here by default.
 * If you have a problem where you need to compute a lot of eigenvalues, but do not need eigenvectors,
 * modify the boolean variable _useEJML below to true and recompile.
 */
...
public class Graph
{
    ...
    private static boolean _useEJML = true;       // uses COLT by default
    ...
    /**
     * Constructor of a graph from graph6 code
     */
        ...
        if (s.charAt(0)==126)
            if (s.charAt(1)<126) {
                ...
                firsti=4;
                    ...
                    // if you end up here, note that n should actually be long, not int!
                n=(s.charAt(2)-63)*1073741824 + (s.charAt(3)-63)*16777216 + (s.charAt(4)-63)*262144 
                 +(s.charAt(5)-63)*4096 + (s.charAt(6)-63)*64 + (s.charAt(7)-63);
                firsti=8;
            }
     ...
     * which is needed for isomorphism testing.
     */
    ...
    /**
     * Returns the tikz description of a graph in the string,
     * to be compiled later with latex to produce a circular drawing of the graph.
     */
    public String tikzFormat() throws IOException {
        double r=2.5;
        StringBuffer buf = new StringBuffer("\\begin{tikzpicture}\n");
        
        // vertex coordinates
        double[] xcoord = new double[n];
        double[] ycoord = new double[n];
        for (int i=0; i<n; i++) {
            xcoord[i] = r*Math.cos((2*Math.PI*i)/n);
            ycoord[i] = r*Math.sin((2*Math.PI*i)/n);
        }
        
        // vertices
        for (int i=0; i<n; i++)
            buf.append("\\fill[black] (" + xcoord[i] + "," + ycoord[i] + ")  circle (6pt);\n");

        // edges
        for (int i=0; i<n; i++)
            for (int j=i+1; j<n; j++)
                if (A[i][j]==1)
                    buf.append("\\draw ("+xcoord[i]+","+ycoord[i]+") -- ("+xcoord[j]+","+ycoord[j]+");\n");
        
        buf.append("\\end{tikzpicture}\n");
        return buf.toString();
    }
}